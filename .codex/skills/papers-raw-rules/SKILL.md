...
- `papers/raw/*.md` is evidence truth; `papers/raw/*.json` is metadata.
- Same-basename `papers/raw/*.pdf` or `papers/raw/*.tex` is the primary source.
...
- Checkpoint 1: MUST run `python papers\scripts\search_a_paper.py DOI`; ONLY one matching JSON with same-basename PDF/TEX passes.
...
- Template: [papers/scripts/add_raw_md.md](../../../papers/scripts/add_raw_md.md); manual: `Motivation`, `Methods`, `Results`, `Meanings`, `Secondary Citations`; auxiliary: `Abstract`, `Gaps`.

### Section-Rules

- Each quotation section in the [papers/scripts/add_raw_md.md](../../../papers/scripts/add_raw_md.md) MUST contain exactly one `claim-type`, `coverage`, and `quotation` HTML declaration.
...
- PRINT: Before every Draft and Re-review, MUST read [papers/scripts/add_raw_md.md](../../../papers/scripts/add_raw_md.md) and print the current section’s `claim-type`, `coverage`, and `quotation` HTML declarations verbatim; NEVER source or infer them from `papers/raw/*.md`.
...
- Write: Write ONLY exact approved content; MUST then run `python papers\scripts\sort_raw_quotations.py mdfile_path`.
