...
Module HFBTHO_collective
  ...
  Public calculate_collective_mass

  Private define_collective_space,energy_moments,S2VCPY,INVMUL,V2SCPY,INVERT,TRACEM,LUDCMP,LUBKSB

  ! Public variables
  Integer(ipr), Parameter, Public :: NDEMOM=3 !< Number of moments
  Integer(ipr), Public, Save :: NOCOMP !< Actual number of independent variables in the inertia tensor
  Integer(ipr), Public, Save :: NOMULT !< Actual number of collective variables
  Integer(ipr), Public, Save :: IIPAIR
  Integer(ipr), Allocatable, Public, Save :: INDMAS(:,:)

  Real(pr), Allocatable, Public, Save :: SK1(:,:) !< Energy moments of order 1; second index: 1 (neutrons), 2 (protons), 3 (total)
  Real(pr), Allocatable, Public, Save :: SK2(:,:) !< Energy moments of order 2; second index: 1 (neutrons), 2 (protons), 3 (total)
  Real(pr), Allocatable, Public, Save :: SK3(:,:) !< Energy moments of order 3; second index: 1 (neutrons), 2 (protons), 3 (total)

  Real(pr), Allocatable, Public, Save :: ATDMAS(:,:) !< ATDHFB mass tensor: second index 1 for neutrons, 2 for protons, 3 for total
  Real(pr), Allocatable, Public, Save :: GCMMAS(:,:) !< GCM mass tensor: second index 1 for neutrons, 2 for protons, 3 for total
  ...
  Real(pr), Allocatable, Private, Save :: COVMET(:,:,:) !< GCM metric; second index: 1 (neutrons), 2 (protons), 3 (total)
  Real(pr), Allocatable, Private, Save :: EMOMET(:,:,:) !< Energy moments of order k=1,.., NDEMOM
...
Contains
  ...
  Subroutine calculate_collective_mass()
    Use HFBTHO, Only : itmin,itmax,lambdaMax

    Integer(ipr) :: it
    Real(pr), Allocatable :: AUX1(:,:),AUX2(:,:),AUX3(:,:)

    Call define_collective_space()

    Allocate(AUX1(1:NOMULT,1:NOMULT),AUX2(1:NOMULT,1:NOMULT),AUX3(1:NOMULT,1:NOMULT))
    ...
    Do it=itmin,itmax
       ...
       AUX1=EMOMET(:,:,1)
       Call S2VCPY(AUX1,NOMULT,NOMULT,SK1(:,it),NOCOMP,NOCOMP)
       AUX2=EMOMET(:,:,2)
       Call S2VCPY(AUX2,NOMULT,NOMULT,SK2(:,it),NOCOMP,NOCOMP)
       AUX3=EMOMET(:,:,3)
       Call S2VCPY(AUX3,NOMULT,NOMULT,SK3(:,it),NOCOMP,NOCOMP)
       ...
       AUX1=EMOMET(:,:,1)
       AUX2=EMOMET(:,:,2)
       Call INVMUL(AUX1,AUX2,AUX3,NOMULT,NOMULT)
       AUX1=0.50_pr*AUX1
       ...
       !> \f]
       Call INVMUL(AUX1,AUX3,AUX2,NOMULT,NOMULT)
       Call S2VCPY(AUX1,NOMULT,NOMULT,ATDMAS(:,it),NOCOMP,NOCOMP)

       ! Make B = M^-1
       Call V2SCPY(ATDMAS(:,it),NOCOMP,NOCOMP,AUX1,NOMULT,NOMULT)
       Call INVERT(AUX1,AUX3,NOMULT,NOMULT)
       ...
       E0_ATD(it)=0.50_pr*TRACEM(AUX3,NOMULT,NOMULT)
       ...
       Call INVERT(AUX1,AUX3,NOMULT,NOMULT)
       AUX2=COVMET(:,:,it)
       Call INVERT(AUX2,AUX3,NOMULT,NOMULT)
       AUX2=MATMUL(AUX2,MATMUL(AUX1,AUX2))
       AUX2=0.25_pr*AUX2
       !
       ! Save M_GCM = B_GCM^(-1)
       Call INVERT(AUX2,AUX3,NOMULT,NOMULT)
       Call S2VCPY(AUX2,NOMULT,NOMULT,GCMMAS(:,it),NOCOMP,NOCOMP)
       !
       ! The ZPE is hbar^2/2 Tr(gamma * B_GCM)
       Call V2SCPY(GCMMAS(:,it),NOCOMP,NOCOMP,AUX1,NOMULT,NOMULT)
       Call INVERT(AUX1,AUX3,NOMULT,NOMULT)
       AUX2=COVMET(:,:,it)
       AUX3=MATMUL(AUX2,AUX1)
       E0_GCM(it) = 0.50_pr*TRACEM(AUX3,NOMULT,NOMULT)
       !
    End Do
    ...
    Call V2SCPY(SK1(:,3),NOCOMP,NOCOMP,AUX1,NOMULT,NOMULT)
    Call V2SCPY(SK2(:,3),NOCOMP,NOCOMP,AUX2,NOMULT,NOMULT)
    ! Total metric
    Call INVMUL(AUX1,AUX2,AUX3,NOMULT,NOMULT)
    ...
    Call V2SCPY(SK1(:,3),NOCOMP,NOCOMP,AUX1,NOMULT,NOMULT)
    Call V2SCPY(SK3(:,3),NOCOMP,NOCOMP,AUX3,NOMULT,NOMULT)
    !
    ! Total ATDHFB mass from the sum of the moments
    Call INVMUL(AUX1,AUX3,AUX2,NOMULT,NOMULT)
    Call S2VCPY(AUX1,NOMULT,NOMULT,ATDMAS(:,3),NOCOMP,NOCOMP)
    ! Make B = M^-1 and get trace of 1/2 * (gamma * B)
    Call V2SCPY(ATDMAS(:,3),NOCOMP,NOCOMP,AUX1,NOMULT,NOMULT)
    Call INVERT(AUX1,AUX3,NOMULT,NOMULT)
    ! Get trace of 1/2 * (gamma * B)
    AUX2=COVMET(:,:,3)
    AUX3=MATMUL(AUX2,AUX1)
    E0_ATD(3)=0.50_pr*TRACEM(AUX3,NOMULT,NOMULT)
    ...
    AUX2=0.25_pr*AUX2
    Call INVERT(AUX2,AUX3,NOMULT,NOMULT)
    Call S2VCPY(AUX2,NOMULT,NOMULT,GCMMAS(:,3),NOCOMP,NOCOMP)
    ! Make B = M^-1 and get trace of 1/2 * (gamma * B)
    Call V2SCPY(GCMMAS(:,3),NOCOMP,NOCOMP,AUX1,NOMULT,NOMULT)
    Call INVERT(AUX1,AUX3,NOMULT,NOMULT)
    AUX2=COVMET(:,:,3)
    AUX3=MATMUL(AUX2,AUX1)
    E0_GCM(3)=0.50_pr*TRACEM(AUX3,NOMULT,NOMULT)

  End Subroutine calculate_collective_mass
  !=======================================================================
  !> This subroutine defines the collective space, that is, the number
  !> of active multipole moments used as constraints (\f$ Q_{10} \f$ does not count)
  !> and the total number of independent matrix elements of the
  !> collective mass tensor.
  !=======================================================================
  Subroutine define_collective_space()
    Use HFBTHO_multipole_moments, Only : numberCons,multLambda

    Integer(ipr) :: icons,jcons,NIMULT,lambda,lambdb,i
    ...
    If(Allocated(INDMAS)) Deallocate(INDMAS,SK1,SK2,SK3,ATDMAS,GCMMAS,COVMET,EMOMET)
    Allocate(INDMAS(1:NOCOMP,1:2))
    Allocate(SK1(1:NOCOMP,1:3),SK2(1:NOCOMP,1:3),SK3(1:NOCOMP,1:3))
    Allocate(ATDMAS(1:NOCOMP,1:3),GCMMAS(1:NOCOMP,1:3))
    Allocate(COVMET(1:NOMULT,1:NOMULT,1:3))
    Allocate(EMOMET(1:NOMULT,1:NOMULT,1:NDEMOM))

    ! Second loop to fill in arrays
    i=0
    Do icons=1,numberCons
       lambda=multLambda(icons)
       If(lambda /= 1) Then
  ...
  !=======================================================================
  !> This routine computes the energy moments \f$ \mathbf{M}^{(K)} \f$, which are defined by
  !> \f[
  !>    M_{ab}^{(K)} = \mathfrak{Re} \sum_{\mu\nu}
  !>      \frac{\langle \mu\nu | \hat{Q}_{a} | 0\rangle
  !>            \langle 0 | \hat{Q}_{b} | \mu\nu\rangle}{(E_{\mu}+E_{\nu})^{K}}.
  !> \f]
  !> In principle the summation over indices \f$\mu, \nu \f$ are unrestricted. In practice, we
  !> run only over half of the qp states, those with positive simplex. An extra factor 2 is thus
  !> added in routine \ref invemo.
  !=======================================================================
  Subroutine energy_moments(it)
    Use HFBTHO, Only : nb,id,kd,ka,RVqpN,RVqpP,RUqpN,RUqpP,REqpN,REqpP,KpwiN,KpwiP,KqpN,KqpP
    Use HFBTHO_multipole_moments, Only : moments_expectation,moments_computeField

    Integer(ipr), Intent(in) :: it !< isospin index (1=neutrons, 2=protons)

    Logical :: debug_robledo, debug_BIII
    Integer(ipr) :: ib,i_uv,nd,nd2,n_qp,k,n2,kk
          ...
          Do ib = 1,nb
             ...
             If(n_qp > 0) Then
                ...
                If(debug_inertia >= 2) Then
                   ...
                   If(ib == nb) Write(6,'("ib=",i2," lambda=",i2," qvala=",f14.7," dd2=",f14.7)') ib,lambda,qvala,dd2
                   lambdb_test=3
                   Call moments_expectation(lambdb_test,it,ib,qvalb,rho,dd1,multMatElems)
                   If(ib == nb) Write(6,'("ib=",i2," lambdb=",i2," qvalb=",f14.7," dd1=",f14.7)') ib,lambdb,qvalb,dd1
                End If
                ...
                If(debug_inertia >= 2) Deallocate(rho)
    ...
    If(debug_inertia >= 1) Then
       ...
       Do i_coll=1,NOMULT
          Do j_coll=1,i_coll
             ...
             lambdb=INDMAS(N_COMP,2)
                                             Factor = one
             If(lambda == 2.And.lambdb == 2) Factor = Factor_q2**2
             If(lambda == 3.And.lambdb == 2) Factor = Factor_q2*Factor_q3
             If(lambda == 3.And.lambdb == 3) Factor = Factor_q3**2
  ...
  !=======================================================================
  !>  This routine calculates the matrix elements
  !>        \f$ \langle 0| \beta_i \beta_j \hat{F} |0 \rangle \f$
  !>  where \f$ |0 \rangle \f$ is the HFB vacuum (possibly under constraints), \f$ \beta_i \f$
  !>  and \f$ \beta_j \f$ are qp-annihilation operators and \f$ \hat{F} \f$ is a one-body
  !>  hermitian operator,
  !>  \f[
  ...
  Subroutine qp_basis_F20(nd,n_qp,Umatr,Vmatr,FOPQP,multMatElems)
    ...
    Real(pr), Allocatable, Intent(Inout) :: multMatElems(:) !< Linearized matrix of the operator F in the HO basis
  ...
  Subroutine print_collective(iw)
    ...
    Real(pr), Allocatable :: AUX1(:,:)
    Real(pr), Allocatable :: GMETRI(:)
    ...
    Allocate(AUX1(1:NOMULT,1:NOMULT))
    Allocate(GMETRI(1:NOCOMP))
    AUX1 = COVMET(:,:,3)
    Call S2VCPY(AUX1,NOMULT,NOMULT,GMETRI,NOCOMP,NOCOMP)
    N_COMP=0
  ...
  !=======================================================================
  !>  This routine linearizes a square symmetric matrix A into a vector V.
  !=======================================================================
  Subroutine S2VCPY(A,N,NA,V,M,MV)
    ...
    If(N > NA .Or. M > MV) Stop 'S2VCPY: N  >  NA; Physical dimensions of A or V exceeded'
    K=0
    Do I=1,N
       Do J=1,I
          K=K+1
  ...
  !=======================================================================
  !>  This routine 'delinearizez' the vector v(:) into a square matrix a(:,:).
  !=======================================================================
  Subroutine V2SCPY(V,M,MV,A,N,NA)
    ...
    If(N > NA .Or. M > MV) Stop 'V2SCPY: N  >  NA; Physical dimensions of A or V exceeded'
  ...
  Subroutine INVMUL(A,B,XA,N,NX)
    ...
    If(N > NX) Stop 'INVMUL: N  >  NX; Physical dimensions of A or B exceeded'
  ...
  Real(pr) Function TRACEM(A,N,NX)
    ...
    If(N > NX) Stop 'TRACEM: Dimension N > NX!'
  ...
  End Function TRACEM
  !=======================================================================
  !>  This subroutine calculates inverse of a matrix. A is the matrix  to be inverted and is
  !>  destroyed in the routine. Upon exit, the matrix A contains the inverse \f$ A^{-1} \f$.
  !>  The inverse is computed with the Cholesky LU decomposition + LU back substitution, which
  !>  are hand-coded in the routines \ref ludcmp and \ref lubksb.
  !=======================================================================
  Subroutine INVERT(A,Y,N,NP)
    ...
    If(N > NP .Or. N > NMAX) Stop 'INVERT: N TOO LARGE. INCREASE NP OR NMAX IN INVERT.'
  ...
  Subroutine LUDCMP(a,n,np,indx,d)
    ...
    Do i=1,n
       ...
       End Do ! j
       If(aamax == 0.) Write(*,*) 'singular matrix in ludcmp'
       vv(i)=1.0_pr/aamax
    End do
    Do j=1,n
       Do i=1,j-1
