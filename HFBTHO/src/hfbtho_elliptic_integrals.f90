...
Module EllipticIntegral
  ...
  Public CompleteEllipticFunction_2nd

  Private auxiliary,elliptic_small_m,nome
...
Contains
  ...
  Real(pr) Function CompleteEllipticFunction_2nd(x)
    ...
    Real(pr) :: Emp,Kmp,Em,Km,qp,x_eff

    If(x < zero .Or. x > one) Stop 'Error in CompleteEllipticFunction_2nd'
  ...
  Subroutine auxiliary(x,Emp,Kmp)
    ...
    Integer(ipr), Parameter :: JE=16, JK=16
    Integer(ipr) :: i
    Real(pr) :: x0, x_eff
  ...
  Real(pr) Function elliptic_small_m(x)
    ...
    If(x >= 0.8_pr .And. x < 0.85_pr) Then
       x0 = 0.825_pr
    ...
    If(x >= 0.85_pr .And. x < 0.9_pr) Then
       x0 = 0.875_pr
