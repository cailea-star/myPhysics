...
! ==================================================================== !
!----------------------------------------------------------------------
!>  This module contains various utility functions to perform large-scale
!>  calculations with HFBTHO, be it full nuclear chart from dripline to
!>  dripline or a subset of nuclei with different deformations. Calculations
!>  of this type require USE_MPI=2; the value USE_MPI=1 is reserved for
!>  parallelized HFB calculations; refer to hfbtho_main.f90 for further
!>  information.
!>
!>  @authors
...
!  Subroutines:
!    * DO_MASSTABLE = 1
!               - read_HFBTHO_MassTable()
!               - allocate_mass_table()
!               - fill_mass_table(icalc)
!               - print_mass_table()
!    * DRIP_LINES = 1
!               - read_HFBTHO_StableLine()
!    * DO_PES = 1
!               - read_HFBTHO_PES()
!----------------------------------------------------------------------
...
#if(DO_MASSTABLE==1)
  Public read_HFBTHO_MassTable,allocate_mass_table,fill_mass_table,print_mass_table
#endif

#if(DRIP_LINES==1)
  Public read_HFBTHO_StableLine
#endif

#if(DO_PES==1)
  Public read_HFBTHO_PES
#endif
...
Contains
...
#if(DO_MASSTABLE==1)
  !=======================================================================
  !> This routine opens and reads the 'hfbtho_MASSTABLE.dat' file that indicates
  !> the nuclei (with different deformations) to be calculated with  HFBTHO
  ...
  !>     deformation (beta)
  !>   - If nRows = 0 (or negative) the rest of the file will be ignored and
  !>     a regular HFBTHO calculation will be made with the parameters given
  !>     in the 'hfbtho_NAMELIST.dat' file.
  !>   - If nRows > zero the first nRows calculations will be made with the
  !>     parameters given in each row. Other input parameters will be take
  !>     from the 'hfbtho_NAMELIST.dat' file.
  !> nRows should not be larger than the actual number of rows on the file.
  !=======================================================================
  Subroutine read_HFBTHO_MassTable()
    Use HFBTHO, Only : Z_masstable,N_masstable,Q20_masstable,beta_masstable,nRows,proton_number,neutron_number,expectation_values,basis_deformation
    ...
    Do i=1,nRows
       Read(lmasstable,*) Z_masstable(i),N_masstable(i),Q20_masstable(i),beta_masstable(i)
    End Do
    Close(lmasstable)

  End Subroutine read_HFBTHO_MassTable
  !=======================================================================
  !> This routine allocates the arrays necessary to construct the output mass table.
  !=======================================================================
  Subroutine allocate_mass_table()
    Use HFBTHO, Only : ierrors_out,Z_out,N_out,Q20_in,beta_in,E_HFB_out,Q20Z_out,Q20N_out,Q20T_out,nRows
  ...
  !=======================================================================
  !> The results from the HFBTHO calculation are stored in arrays. This subroutine is used in
  !> when the mass table is calculated without MPI parallelism.
  !=======================================================================
  Subroutine fill_mass_table(icalc)
    Use HFBTHO, Only : ierrors_out,Z_out,N_out,Q20_in,beta_in,E_HFB_out,Q20Z_out,Q20N_out,Q20T_out,iRow,kindhfb_INI,ehfb,etot,ierror_flag, &
                       Z_masstable,N_masstable,Q20_masstable,beta_masstable
    USe HFBTHO_multipole_moments, Only : qmoment

    Integer(ipr), intent(in) :: icalc !< Calculations performed so far
  ...
  !=======================================================================
  !> This routine prints the output mass table to the 'MassTableOut.dat' file and on screen.
  !=======================================================================
  Subroutine print_mass_table()
    Use HFBTHO, Only : mpi_taskid,nRows,ierrors_out,Z_out,N_out,Q20_in,beta_in,E_HFB_out,Q20Z_out,Q20N_out,Q20T_out
...
#if(DRIP_LINES==1)
  !=======================================================================
  !> This subroutine reads the location of the valley of stability as well as the direction
  !> (proton or neutron) in which dripline calculations should proceed.
  !=======================================================================
  Subroutine read_HFBTHO_StableLine()
    Use HFBTHO, Only : Z_stable_line,N_stable_line,direction_sl,nRows,proton_number,neutron_number
...
#if(DO_PES==1)
  !=======================================================================
  !> This subroutine defines a potential energy surface (PES) by reading the file hfbtho_PES.dat.
  !> This routine is only relevant when the DO_PES flag is activated
  !=======================================================================
  Subroutine read_HFBTHO_PES()
    Use HFBTHO, Only : Z_PES,N_PES,lambda_PES,Q_PES,bet2_PES,bet3_PES,bet4_PES,npoints,ndefs,nRows,n_real_masstable_in, &
                       proton_number,neutron_number,expectation_values,beta2_deformation,beta3_deformation,beta4_deformation
...
#if(USE_MPI>0)
!----------------------------------------------------------------------
!>  This module contain utility functions and subroutines to exchange information
!>  among MPI processes involved in a parallel calculation.
!>
!> @authors
!> Rodrigo Navarro Perez, Nicolas Schunck, Jared O'Neal
!----------------------------------------------------------------------
!  Subroutines: - configure_MPI_environment(my_comm_world,my_comm_team,my_n_teams,my_team_color,ierr)
!               - Create_MPI_Teams
!               - Construct_Vectors
!               - broadcast_vectors
...
Module HFBTHO_mpi_communication
  ...
  Public configure_MPI_environment,Create_MPI_Teams,Construct_Vectors,broadcast_vectors,Deconstruct_Vectors, &
         pack,unpack,broadcast_binary_to_team

  Private allocate_mpi_vectors

  Integer(ipr), Private, Save :: debug = 0 !< Internal flag to provide debugging information

Contains
  !=======================================================================
  !> This routine configures the MPI setup of HFBTHO using the given MPI information, which usually
  !> has been setup by the application using HFBTHO. Note that the ranks that indicate which process
  !> is the lead process in both the world and team communicators are fixed by HFBTHO. Therefore
  !> applications should use these rather than making their own decision.
  !
  !> @param[in] my_comm_world - the MPI communicator that contains all MPI processes that can be used
  !>                            by HFBTHO for computations.
  !> @param[in] my_comm_team - the MPI communicator of the process' team
  !> @param[in] my_n_teams - the total number of teams in existence
  !> @param[in] my_team_color - the unique ID of the process' team
  !> @param[inout] ierr - the MPI error code.  MPI_SUCCESS if configuration successful. Otherwise, the
  !>                      MPI error code of the first MPI error detected.
  !=======================================================================
  Subroutine configure_MPI_environment(my_comm_world, my_comm_team, my_n_teams, my_team_color, ierr)
      Use mpi,    Only : MPI_SUCCESS
      Use HFBTHO, Only : WORLD_LEAD_RANK, TEAM_LEAD_RANK, COMM_world, mpi_taskid, mpi_size, &
                         COMM_team, number_teams, team_color, team_rank, team_size,HFB_cores
      ...
      ! HFBTHO's world communicator
      COMM_world = my_comm_world
      Call MPI_Comm_size(COMM_world, mpi_size, ierr)
      If(ierr /= MPI_SUCCESS) Then
         Write(*,'("Error 1")')
         Return
      End If

      Call MPI_Comm_rank(COMM_world, mpi_taskid, ierr)
      If(ierr /= MPI_SUCCESS) Then
         Write(*,'("Error 2")')
         Return
      End If
      ...
      ! Team communicator
      COMM_team = my_comm_team
      number_teams = my_n_teams
      team_color = my_team_color
      Call MPI_Comm_size(COMM_team, team_size, ierr)
      If(ierr /= MPI_SUCCESS) Then
         Write(*,'("Error 3")')
         Return
      End If
      ...
      Call MPI_Comm_rank(COMM_team, team_rank, ierr)
      If(ierr /= MPI_SUCCESS) Then
         Write(*,'("Error 4")')
         Return
      End If
      ...
      If(debug>0) Then
         If(mpi_taskid == WORLD_LEAD_RANK) Then
            Write(*,"(A,I3,A,I3,3A,I0,A)") "[", mpi_taskid, "/", team_rank, "]", &
                                           " I am the world leader and", &
                                           " there are ", mpi_size, &
                                           " processes in the world"
         End If
         If(team_rank == TEAM_LEAD_RANK) Then
            Write(*,"(A,I3,A,I3,3A,I0,A,I0)") "[", mpi_taskid, "/", team_rank, "]", &
                                              " I am a team leader", &
                                              " and there are ", team_size, &
                                              " processes in team ", team_color
         Else
            Write(*,"(A,I3,A,I3,2A,I0)") "[", mpi_taskid, "/", team_rank, "]", &
                                         " I am a worker in team ", team_color
         End If
      End If

  End Subroutine configure_MPI_environment
  !=======================================================================
  !> This routine creates new communicators for teams of MPI processes to calculate each nucleus
  !> with different deformations. The processes are distributed as evenly as possible and each
  !> team will have at most as many members as the number of deformations that will be used
  !> (having more members would be inefficient). A group and communicator are created with one
  !> member of each team that will act as team leader and communicate with a 'super-leader' to
  !> distribute the nucleus to be calculated by each group.
  !=======================================================================
  Subroutine Create_MPI_Teams(n_cores)
    Use HFBTHO, Only : COMM_world,COMM_team,ierr_mpi,mpi_taskid,mpi_size,team_color,number_teams,team_size,team_rank
    Use mpi

    Integer(ipr), Intent(In) :: n_cores !< - Number of cores for a single team
    ...
    Call MPI_COMM_SPLIT(COMM_world,team_color,mpi_taskid,COMM_team,ierr_mpi)
    ...
    If(debug > 0) Then
       If(mpi_taskid == 0) Write(*,'("mpi_size = ",i6," number of teams",i6)') mpi_size,number_teams
       Write(*,'("Process",i6," has rank ",i6," in team of color",i6)') mpi_taskid,team_rank,team_color
    End If

  End Subroutine Create_MPI_Teams
  !=======================================================================
  !> After the world master process has read the 'hfbtho_NAMELIST.dat' and, optionally, the
  !> 'hfbtho_MASSTABLE.dat' files, it constructs (allocates and fills out) a set of vectors to
  !> be broadcast to the other processes. These vectors contain the data of the input files:
  ...
  Subroutine Construct_Vectors()
    Use HFBTHO, Only : lambdaMax,n_int_masstable_in,nRows,n_real_masstable_in,vector_sizes,vector_int_mpi,vector_real_mpi,vector_log_mpi, &
                       number_of_shells,oscillator_length,basis_deformation,proton_number,neutron_number,type_of_calculation, &
                       beta2_deformation,beta3_deformation,beta4_deformation,number_iterations,accuracy,restart_file,functional, &
                       add_initial_pairing,type_of_coulomb,include_3N_force,user_pairing,vpair_n,vpair_p,pairing_cutoff,pairing_feature, &
                       lambda_values,lambda_active,expectation_values,proton_blocking,neutron_blocking,switch_to_THO,projection_is_on, &
                       gauge_points,delta_Z,delta_N,set_temperature,temperature,collective_inertia,fission_fragments,pairing_regularization, &
                       automatic_basis,localization_functions,canonical_wavefunctions,filter,fragment_properties, &
                       set_neck_constrain,neck_value,number_Gauss,number_Laguerre,number_Legendre,compatibility_HFODD,number_states, &
                       force_parity,write_hel,print_time,PNP_is_on,number_of_gauge_points,delta_neutrons,delta_protons, &
#if(DO_MASSTABLE==1)
                       Z_masstable,N_masstable,Q20_masstable,beta_masstable, &
#endif
#if(DO_PES==1)
                       Z_PES,N_PES,lambda_PES,Q_PES,bet2_PES,bet3_PES,bet4_PES,npoints,ndefs, &
#endif
#if(DRIP_LINES==1)
                       Z_stable_line,N_stable_line,direction_sl, &
#endif
                       AMP_is_on,number_of_rotational_angles,maximal_angular_momentum

    Integer(ipr) :: i,j,ntot,ncons
    ...
    vector_sizes(3) = 13 ! boolean
    vector_sizes(4) =  nRows
#if(DO_PES==1)
    vector_sizes(5) =  ndefs
#endif
    If(Allocated(vector_int_mpi)) Deallocate(vector_int_mpi,vector_real_mpi,vector_log_mpi)
    ...
    ntot = ntot + 1
    vector_log_mpi(ntot) = canonical_wavefunctions
    ntot = ntot + 1
  ...
  !=======================================================================
  !> This routine is used to broadcast the input data to non-master processors. There are two main
  !> use cases:
  !>  - USE_MPI=1: This corresponds to parallelized HFB calculations within a single team. When HFBTHO
  !>               is used through its Python binding and called from a Python script, there can be
  !>               multiple teams processing simultaneously different configurations. In this case,
  !>               the team leader reads the input data from the HFBTHO_NAMELIST.dat file and broadcasts
  !>               it within its own team.
  !>  - USE_MPI=2: This corresponds to mass tables, driplines or PES calculations as hard-coded in the
  !>               Fortran. In this case, the data from the HFBTHO_NAMELIST.dat file is read from the
  !>               world master process and broadcast to all world processes.
  !=======================================================================
  Subroutine broadcast_vectors()
#if(USE_MPI==1)
    Use HFBTHO, Only : vector_sizes,vector_int_mpi,vector_real_mpi,vector_log_mpi,functional,team_rank, &
                       COMM_team,ierr_mpi,nRows
#else
#if(DO_PES==1)
    Use HFBTHO, Only : functional,vector_sizes,vector_int_mpi,vector_real_mpi,vector_log_mpi,mpi_taskid,COMM_world,ierr_mpi,nRows,npoints,ndefs
#else
    Use HFBTHO, Only : functional,vector_sizes,vector_int_mpi,vector_real_mpi,vector_log_mpi,mpi_taskid,COMM_world,ierr_mpi,nRows
#endif
#endif
    Use mpi

    ! USE_MPI=1. This mode spreads a single HFB calculation across multiple MPI ranks. This mode is also
    !            compatible with a HFBTHO library, hence a Python binding where the code is called as a
    !            subroutine. Broadcast is performed within a team only
#if(USE_MPI==1)
    If(team_rank /= 0) Then
       If(Allocated(vector_sizes)) Deallocate(vector_sizes)
       Allocate(vector_sizes(1:4))
    End If
    Call mpi_bcast(vector_sizes,4,mpi_integer,0,COMM_team,ierr_mpi)
    If(team_rank /= 0) Then
       nRows = vector_sizes(4)
       Call allocate_mpi_vectors()
    End If
    Call mpi_bcast( vector_int_mpi,vector_sizes(1),mpi_integer,0,COMM_team,ierr_mpi)
    Call mpi_bcast(vector_real_mpi,vector_sizes(2),mpi_double_precision,0,COMM_team,ierr_mpi)
    Call mpi_bcast( vector_log_mpi,vector_sizes(3),mpi_logical,0,COMM_team,ierr_mpi)
    Call mpi_bcast(functional,30,mpi_character,0,COMM_team,ierr_mpi)
#else
    ! USE_MPI=2. This mode does not allow spreading a single HFB calculation across multiple MPI ranks and
    !            is not compatible with a HFBTHO library (only stand-alone program). Broadcast is performed
    ...
    If(mpi_taskid > 0) Then
       ...
       Allocate(vector_sizes(1:5))
    End If
    Call mpi_bcast(vector_sizes,5,mpi_integer,0,COMM_world,ierr_mpi)
    If(mpi_taskid > 0) Then
    ...
    If(mpi_taskid > 0) Then
       ...
       Allocate(vector_sizes(1:4))
    End If
    Call mpi_bcast(vector_sizes,4,mpi_integer,0,COMM_world,ierr_mpi)
    If(mpi_taskid > 0) Then
       nRows = vector_sizes(4)
       Call allocate_mpi_vectors()
    End If
#endif
    Call mpi_bcast( vector_int_mpi,vector_sizes(1),mpi_integer,0,COMM_world,ierr_mpi)
    Call mpi_bcast(vector_real_mpi,vector_sizes(2),mpi_double_precision,0,COMM_world,ierr_mpi)
    Call mpi_bcast( vector_log_mpi,vector_sizes(3),mpi_logical,0,COMM_world,ierr_mpi)
    Call mpi_bcast(functional,30,mpi_character,0,COMM_world,ierr_mpi)
#endif

  End Subroutine broadcast_vectors
  !=======================================================================
  !> This routine allocate the arrays that will be received from the master's broadcast; see
  !> \ref broadcast_vectors() for additional information.
  !=======================================================================
  Subroutine allocate_mpi_vectors()
#if(DO_MASSTABLE==1)
    Use HFBTHO, Only : vector_sizes,vector_int_mpi,vector_real_mpi,vector_log_mpi,Z_masstable,N_masstable,Q20_masstable,beta_masstable,nRows
#elif(DO_PES==1)
    Use HFBTHO, Only : vector_sizes,vector_int_mpi,vector_real_mpi,vector_log_mpi,Z_PES,N_PES,lambda_PES,Q_PES,bet2_PES,bet3_PES,bet4_PES,npoints,ndefs
#elif(DRIP_LINES==1)
    Use HFBTHO, Only : vector_sizes,vector_int_mpi,vector_real_mpi,vector_log_mpi,Z_stable_line,N_stable_line,direction_sl,nRows
#else
    Use HFBTHO, Only : vector_sizes,vector_int_mpi,vector_real_mpi,vector_log_mpi
#endif
  ...
  !=======================================================================
  !> This routine takes the broadcast vectors and reconsitutes the input data that were read from
  !> the master process; see \ref broadcast_vectors() for additional information.
  !=======================================================================
  Subroutine Deconstruct_Vectors()
    Use HFBTHO, Only : lambdaMax,vector_int_mpi,vector_real_mpi,vector_log_mpi, &
                       number_of_shells,oscillator_length,basis_deformation,proton_number,neutron_number,type_of_calculation, &
                       beta2_deformation,beta3_deformation,beta4_deformation,number_iterations,accuracy,restart_file,functional, &
                       add_initial_pairing,type_of_coulomb,include_3N_force,user_pairing,vpair_n,vpair_p,pairing_cutoff,pairing_feature, &
                       lambda_values,lambda_active,expectation_values,proton_blocking,neutron_blocking,switch_to_THO,projection_is_on, &
                       ...
                       force_parity,write_hel, print_time,PNP_is_on,number_of_gauge_points,delta_neutrons,delta_protons, &
#if(DO_MASSTABLE==1)
                       n_int_masstable_in,n_real_masstable_in,Z_masstable,N_masstable,Q20_masstable,beta_masstable,nRows, &
#endif
#if(DO_PES==1)
                       n_int_masstable_in,Z_PES,N_PES,lambda_PES,Q_PES,bet2_PES,bet3_PES,bet4_PES,npoints,ndefs, &
#endif
#if(DRIP_LINES==1)
                       n_int_masstable_in,Z_stable_line,N_stable_line,direction_sl,nRows, &
#endif
                       AMP_is_on,number_of_rotational_angles,maximal_angular_momentum

    Integer(ipr) :: i,j,ntot
    ...
    ntot = ntot + 1
    canonical_wavefunctions= vector_log_mpi(ntot)
    ntot = ntot + 1
...
#if(DRIP_LINES==1)
  !=======================================================================
  !> This routine finds and broadcasts the value of the minimum energy for a given nucleus.
  !=======================================================================
  Subroutine find_minimum_energy()
    Use mpi
    Use HFBTHO, Only : Energy_Chain,Energy_Chain_gthr,COMM_team,ierr_mpi,team_rank,Minimum_Energy
    ...
    If(team_rank == 0) Minimum_Energy = minval(Energy_Chain_gthr,1)
...
#if(DO_MASSTABLE==1)
  !=======================================================================
  !> This routine allocates the vectors that will be gathered by the master in mass table calculations.
  !=======================================================================
  Subroutine allocate_out_vectors()
    Use HFBTHO, Only : vector_sizes,vector_int_mpi,vector_real_mpi,vector_log_mpi,nRows,nRows_task,mpi_size,mpi_taskid, &
                       n_int_masstable_out,n_real_masstable_out

    Deallocate(vector_sizes,vector_int_mpi,vector_real_mpi,vector_log_mpi)
    nRows_task = nRows/mpi_size
    If(mpi_taskid > 0 .And. mpi_taskid <= Mod(nRows,mpi_size)) nRows_task = nRows_task + 1
  ...
  !=======================================================================
  !> After each HFBTHO calculation, each process saves the results in vectors that will be gathered
  !> later by the master in mass table calculations.
  !=======================================================================
  Subroutine fill_out_vectors(icalc)
    Use HFBTHO, Only : vector_int_mpi,vector_real_mpi,n_int_masstable_out,n_real_masstable_out,ierror_flag,iRow, &
                       Z_masstable,N_masstable,Q20_masstable,beta_masstable,ehfb,etot,kindhfb_INI
    Use HFBTHO_multipole_moments, Only : qmoment

    Integer(ipr), Intent(In) :: icalc !< - Current index of computed nucleus
  ...
  !=======================================================================
  !> The master process gathers the result from all calculations and fills out the resulting
  !> mass table including number of protons, number of neutrons, input quadrupole moment, basis
  !> deformation, HFB energy, output quadrupole moment from protons, neutrons and total.
  !=======================================================================
  Subroutine gather_results()
    Use mpi
    Use HFBTHO, Only : mpi_taskid,mpi_size,COMM_world,ierr_mpi,disp_int,disp_real,vector_sizes_gthr,vector_sizes_int_gthr, &
                       vector_sizes_real_gthr,vector_int_gthr,vector_real_gthr,vector_disp_int_gthr,vector_disp_real_gthr,nRows, &
                       vector_sizes,vector_int_mpi,vector_real_mpi,n_int_masstable_out,n_real_masstable_out,Z_out,N_out,ierrors_out, &
                       Q20_in,beta_in,E_HFB_out,Q20Z_out,Q20N_out,Q20T_out
    ...
    End If
    Call mpi_gather(vector_sizes,2,mpi_integer,vector_sizes_gthr,2,mpi_integer,0,COMM_world,ierr_mpi)
    ...
    Call mpi_gatherv(vector_int_mpi,vector_sizes(1),mpi_integer,vector_int_gthr,vector_sizes_int_gthr,vector_disp_int_gthr,&
                     mpi_integer,0,COMM_world,ierr_mpi)

    Call mpi_gatherv(vector_real_mpi,vector_sizes(2),mpi_double_precision,vector_real_gthr,vector_sizes_real_gthr,&
                     vector_disp_real_gthr,mpi_double_precision,0,COMM_world,ierr_mpi)
...
#endif
  !=======================================================================
  !> When distributing a single HFB calculation across several MPI ranks, the binary file can only
  !> be accessed by the team leader (either read or write) to avoid conflicts. After the master
  !> process has read the binary file, it constructs (allocates and fills out) a set of vectors
  !> containing all the data to be broadcast to the other processes in the team:
  ...
  !> The size of each vector must also be broadcast since other MPI processes don't know about it
  !> before. This routine is the analogue of \ref construct_vectors(), only for the content of the
  !> binary file instead of that of the input data file.
  !=======================================================================
  Subroutine pack()
    Use UNEDF, Only : use_INM,use_cm_cor,use_j2terms,force_is_dme,finite_range,hb0_charge_dependent,hbzero, &
                      E_NM,K_NM,SMASS_NM,RHO_NM,ASS_NM,LASS_NM,VMASS_NM,P_NM,KA_NM,Crho,Cdrho,Ctau,CrDr,CrdJ,CJ,CpV0,CpV1,sigma
    Use HFBTHO_Gauss, Only : nghl
    Use HFBTHO_Lipkin, Only : ala2
    Use HFBTHO_multipole_moments, Only : multLag
    Use HFBTHO, Only : vector_sizes,vector_int_mpi,vector_real_mpi,vector_log_mpi, &
                       lambdaMax,bloall,nqp,nuv,functional,bloblo,blo123,blok1k2,blomax,pwi,hb0,hb0n,hb0p,b0,bz,bp,si,etot,rms, &
                       bet,xmix,del,ept,ala,alast,tz,varmas,varmasNZ,pjmassNZ,ass,skass,neckLag,qfield,ro,aka, &
                       vn,vhbn,vrn,vzn,vdn,vsn,vSFIZn,vSZFIn,vSFIRn,vSRFIn,dvn,vp,vhbp,vrp,vzp,vdp,vsp,vSFIZp,vSZFIp,vSFIRp,vSRFIp,dvp, &
                       bloqpdif,temper,entropy,fp_T,fn_T,MEFFn,MEFFp,geff_inv

    Integer(ipr) :: i, k, ihl, n_bool, n_int, n_real
    ...
    If(debug > 0) Write(*,'("n_bool = ",i6," expected = ",i6)') i,n_bool
    ...
    If(debug > 0) Write(*,'("n_int = ",i6," expected = ",i6)') i, n_int
    ...
    If(debug > 0) Write(*,'("n_real = ",i6," expected = ",i6)') i, n_real

  End Subroutine pack
  !=======================================================================
  !> When distributing a single HFB calculation across several MPI ranks, the binary file can only
  !> be accessed by the team leader (either read or write) to avoid conflicts. After the master
  !> process has broadcast the content of the binary file to all team members, each of them
  !> reconstructs locally the data, i.e., the content of the binary file. This routine is the
  !> analogue of \ref deconstruct_vectors(), only for the binary file.
  !=======================================================================
  Subroutine unpack()
    Use UNEDF, Only : use_INM,use_cm_cor,use_j2terms,force_is_dme,finite_range,hb0_charge_dependent,hbzero, &
                      E_NM,K_NM,SMASS_NM,RHO_NM,ASS_NM,LASS_NM,VMASS_NM,P_NM,KA_NM,Crho,Cdrho,Ctau,CrDr,CrdJ,CJ,CpV0,CpV1,sigma
    ...
    Use HFBTHO_Lipkin, Only : ala2
    Use HFBTHO_multipole_moments, Only : multLag
    Use HFBTHO, Only : vector_int_mpi,vector_real_mpi,vector_log_mpi, &
                       lambdaMax,bloall,nqp,nuv,functional,bloblo,blo123,blomax,pwi,hb0,hb0n,hb0p,b0,bz,bp,si,siold,etot,rms, &
                       blok1k2,bet,xmix,del,ept,ala,alast,tz,varmas,varmasNZ,pjmassNZ,ass,skass,neckLag,qfield,ro,aka, &
                       vn,vhbn,vrn,vzn,vdn,vsn,vSFIZn,vSZFIn,vSFIRn,vSRFIn,dvn,vp,vhbp,vrp,vzp,vdp,vsp,vSFIZp,vSZFIp,vSFIRp,vSRFIp,dvp, &
                       bloqpdif,temper,entropy,fp_T,fn_T,MEFFn,MEFFp,geff_inv
  ...
  !=======================================================================
  !> This routine broadcasts the content of the binary file to team members. Within each team,
  !> the master process broadcasts first the vector_sizes arrays so that the other processes
  !> in the team can do all allocations. Then the master process broadcasts integers, reals,
  !> logicals and a single string. This is the analogue of \ref broadcast_vectors(), only for
  !> binary file.
  !=======================================================================
  Subroutine broadcast_binary_to_team(n_sizes)
    Use HFBTHO, Only : vector_sizes,vector_int_mpi,vector_real_mpi,vector_log_mpi,functional, &
                       COMM_team,ierr_mpi,mpi_taskid,team_rank
    Use mpi

    Integer(ipr), Intent(In) :: n_sizes !< - Number of different types of dat to be broadcast

    Integer(ipr) :: n_bool, n_int, n_real
    ...
    If(debug > 1) Then
       Write(*,'("bcast_binary: mpi_taskid = ",i4," team_rank = ",i4)') mpi_taskid,team_rank
       Write(*,'("bcast_binary: n_bool = ",i6," n_int = ",i6," n_real = ",i6)') n_bool,n_int,n_real
    End If
