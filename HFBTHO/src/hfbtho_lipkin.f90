...
Module HFBTHO_Lipkin
  ...
  Public tracesln,tracesln_qp,densitln

  Private

  Real(pr), Public, Save :: ala2(2),etr(3),ssln(3,2),Geff(2)

Contains
  !=======================================================================
  !> This routine computes the Lipkin-Nogami parameter \f$ \lambda_2 \f$ for a monopole
  ...
  !=======================================================================
  Subroutine tracesln()
    Use HFBTHO, Only : itmin,itmax,nb,ka,kd,id,lcanon,ala,ept,del,do_print
    Use HFBTHO_canonical, Only : ek,vk

    Integer(ipr) :: iw,it,ib,k1,k2,kkk,k
    Real(pr) :: AAV,SNtor,SDtor,glimit,gsum,gg
    ...
    Do it=itmin,itmax
       ...
       ! For \Delta -> 0, we take G = 2 / \sum{k>0} ( 1 /\sqrt{(e_k - \lambda)^2} ) with e_k s.p. (canonical) energy
       glimit = zero
       If(Abs(SDtor) > 1.e-13_pr) Then
          Geff(it) = del(it)**2/ept(it)
          ala2(it) = -Geff(it)*(SNtor/SDtor)
       Else
          If(Abs(ept(it)) < 1.e-13_pr) Then
             gsum = zero
             Do ib=1,nb
                k1=ka(ib,it)+1; k2=ka(ib,it)+kd(ib,it)
                If(k1 <= k2) Then
                   ...
                   Do k=1,id(ib)
                      kkk=kkk+1
                      gsum = gsum + Sqrt( (ek(kkk,it) - ala(it))**2 )
                   End Do
                End If
             End Do
             glimit = - two / gsum
             gg = Log(two) - Log(gsum) + Log(Sntor) -Log(SDtor)
             ala2(it) = -Exp(gg)
          Else
             gg = two*Log(Abs(del(it))) - Log(Abs(ept(it))) + Log(Sntor) - Log(SDtor)
             ala2(it) = -Exp(gg)
          End If
       End If
       If(ala2(it) >= 10.0_pr) ala2(it)=4.0_pr ! ala2 goes to hell
       etr(it)=-four*ala2(it)*S_U2V2           ! total energy for current particle number
    End Do !it
    etr(3)=etr(1)+etr(2) ! total energy

    If(do_print == 1) Then
       Do iw=lout,lfile
          Write(iw,'(26x,a,2(1x,f7.3),a,3(1x,f9.3),a,2(1x,f7.3))') '  f: ala2(n,p)=',ala2,' #eln(n,p,t)=',etr,' #del+ala2=',del+ala2
       End Do
    End If
  ...
  Subroutine tracesln_qp()
    Use HFBTHO, Only : itmin,itmax,nb,id,nbx,rk,ak,del,ept,do_print
    ...
    Do it=itmin,itmax
       ...
       If(ala2(it) >= 10.0_pr) ala2(it)=4.0_pr  ! in case ala2 goes to hell
       etr(it)=-four*ala2(it)*etr(it)           ! total energy
    End Do
    etr(3)=etr(1)+etr(2)         ! total energy
  ...
  !=======================================================================
  !> This routine calculates the densities in r-space at Gauss-mesh points
  !> corrected due to Lipkin-Nogami.
  !=======================================================================
  Subroutine densitln()
    Use HFBTHO_Gauss, Only : ngh,ngl,nghl,ql,qh,wdcori
    Use HFBTHO_canonical, Only : vk,ddc
    Use HFBTHO, Only : itmin,itmax,nb,ka,kd,id,ia,nz,nr,nl,ns,ro
    ...
    Do ib=1,nb
       ...
       nd=id(ib); i0=ia(ib)
       Do n2=1,nd
          i02=i0+n2; nzb=nz(i02); nrb=nr(i02); nlb=nl(i02); nsb=ns(i02)
          Do n1=1,n2
             i01=i0+n1; nza=nz(i01); nra=nr(i01); nla=nl(i01); nsa=ns(i01)
             k0=k00
             Do it=itmin,itmax
                k1=ka(ib,it)+1
