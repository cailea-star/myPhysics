/**
 * @file    hfb_cylindrical_field.cpp
 * @author  cailea
 * @date    2026-05-05
 * @brief   Calculate cylindrical Skyrme and pairing fields.
 */
...
namespace cylindrical_field_detail {
...
double regularize_gr_at_point(double EspCut_F, double lambda_F, double vcent_F, double vmass_F, double gr_F) {
    ...
    if (kLow2_F > 0.0) {
        ...
        const double low_term_F = 0.25 * (kLow_F / (std::numbers::pi * std::numbers::pi * vmass_F)) * (1.0 - 0.5 * (kFermi_F / kLow_F) * std::log((kFermi_F + kLow_F) / (kFermi_F - kLow_F)));
        const double high_term_F = 0.25 * (kHigh_F / (std::numbers::pi * std::numbers::pi * vmass_F)) * (1.0 - 0.5 * (kFermi_F / kHigh_F) * std::log((kHigh_F + kFermi_F) / (kHigh_F - kFermi_F)));
        ...
        const double high_term_F = 0.25 * (kHigh_F / (std::numbers::pi * std::numbers::pi * vmass_F)) * (1.0 + (kFermi_F / kHigh_F) * std::atan(kFermi_F / kHigh_F));
        gr_F = 1.0 / (1.0 / gr_F - high_term_F);
    }
    return gr_F;
}
...
void HFBCylindricalField::set_zero() {
