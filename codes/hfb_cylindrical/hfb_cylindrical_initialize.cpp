/**
 * @file    hfb_cylindrical_initialize.cpp
 * @author  cailea
 * @date    2026-05-05
 * @brief   Initialize cylindrical HFB Woods-Saxon fields.
 */
...
struct WSShapeGeometry {
    ...
    WSGeometry calc_geometry(double r2_F, double z_F) const {
        ...
        const double cos_F = z_shift_F / std::sqrt(distance2_F);
        const double P2_F = 0.5 * (3.0 * cos2_F - 1.0);
        const double P3_F = 0.5 * (5.0 * cos_F * cos2_F - 3.0 * cos_F);
...
void HFBKramersNucleusCylindrical::initialize_WS_field() {
    ...
    const auto& z_F1D_z = cylindricalbasis.z_F1D_z;
    const auto& r_F1D_r = cylindricalbasis.r_F1D_r;

    field_neutron.set_zero();
    field_proton.set_zero();

    // (V_0,G_{WS}) → (v_{cent},v_{mass},v_{∇J},Δ).
    const auto initialize_field_Func = [&](HFBCylindricalField& field_, double V0WS_q_F, double hbzero_q_F, double V0LS_q_F) {
    ...
    initialize_field_Func(field_neutron, V0WS_n_F, active_edf_.hbzeron_F, V0LS_n_F);
    initialize_field_Func(field_proton, V0WS_p_F, active_edf_.hbzerop_F, V0LS_p_F);

    // P_{WS} → stdout.
    std::cout << "[HFBKramersNucleusCylindrical::initialize_WS_field] Woods-Saxon initialization\n";
