/**
 * @file    hfb_cylindrical_observable.hpp
 * @author  cailea
 * @date    2026-09-12
 * @brief   Calculate cylindrical HFB observables.
 */
...
#include "hfb_kramers_blocking.hpp"

class HFBKramersNucleusCylindrical;
class HFBKramersBlocking;

/**
 * @brief Store cylindrical HFB observables.
 */
class HFBCylindricalObservable {
public:
...
public:
    /**
     * @brief Calculate cylindrical Kramers HFB observables by quadrature.
     * @math (HFB,B_n,B_p) → (A,E,β₂,r_rms,E_pair,Δ,λ,λ₂).
     * @output Updated observables for the supplied densities and fields.
     * @note Blocking trackers determine the last occupied energy estimate.
     */
    void update_observable(const HFBKramersNucleusCylindrical& hfb_, const std::vector<HFBKramersBlocking>& neutronBlockings_ = {}, const std::vector<HFBKramersBlocking>& protonBlockings_ = {});
};

namespace hfb_cylindrical_observable_detail {
 ...
 */
inline MatrixTrace calc_matrix_trace(const HFBKramers& hfb_) {
    MatrixTrace trace_;

    // Kramers partners give equal particle-hole and pairing traces.
    for (int block_I = 0; block_I < hfb_.Nblock_I; ++block_I) {
        ...
        trace_.Eph_F += field.GammaPosPos_F2D_bsp_bsp.cwiseProduct(solution.rhoPosPos_F2D_bsp_bsp).sum();
        trace_.Epair_F += field.DeltaPosNeg_F2D_bsp_bsp.cwiseProduct(solution.kappaPosNeg_F2D_bsp_bsp).sum();
        // Δρ_weighted=Σ_ab η_b Δ⁺⁻_ab ρ⁺⁺_ab.
        trace_.DeltaRho_F += (field.DeltaPosNeg_F2D_bsp_bsp * solution.eta_F1D_bsp.asDiagonal()).cwiseProduct(solution.rhoPosPos_F2D_bsp_bsp).sum();
    }
...
inline LocalEnergyTrace calc_local_energy_trace(const HFBKramersNucleusCylindrical& hfb_, const EDFParamsSkyrme& edf_skyrme_) {
    ...
    const auto& field_n_ = hfb_.field_neutron;
    const auto& field_p_ = hfb_.field_proton;
...
inline double estimate_last_lambda(const HFBKramers& blocklist_, const std::vector<HFBKramersBlocking>& activeBlockings_, double Nparticle_F_, double pairTrace_F_) {
    ...
    // E_{sp}=λ+E_{bqp}(1-2V²).
    for (int block_I = 0; block_I < blocklist_.Nblock_I; ++block_I) {
        const auto& block_ = blocklist_.solutions[block_I];
        ...
        for (int bqp_I = 0; bqp_I < static_cast<int>(block_.Eqp_F1D_bqp.size()); ++bqp_I) {
            ...
            // B_μ → V_μ²=1/2.
            for (const HFBKramersBlocking& activeBlocking_ : activeBlockings_) {
                if (activeBlocking_.block_I == block_I && activeBlocking_.bqp_I == bqp_I) {
                    V2_F = 0.5;
                }
            }
...
inline void HFBCylindricalObservable::update_observable(const HFBKramersNucleusCylindrical& hfb_, const std::vector<HFBKramersBlocking>& neutronBlockings_, const std::vector<HFBKramersBlocking>& protonBlockings_) {
    using namespace hfb_cylindrical_observable_detail;
    const auto& density_n_ = hfb_.density_neutron;
    const auto& density_p_ = hfb_.density_proton;
    const int Nr_I = hfb_.cylindricalsetting.Nr_I;
    const int Nz_I = hfb_.cylindricalsetting.Nz_I;
