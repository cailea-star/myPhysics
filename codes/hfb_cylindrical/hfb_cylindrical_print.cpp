...
void print_blocking(const CylindricalSetting& cylindricalsetting_, const HFBKramersBlocking& blocking_, bool isNeutron_B) {
    if (blocking_.block_I < 0 || blocking_.block_I >= static_cast<int>(cylindricalsetting_.labels_S2D_block_bsp.size())) {return;}
    if (blocking_.bqp_I < 0) {return;}
    const auto& labels_S1D_bsp = cylindricalsetting_.labels_S2D_block_bsp[blocking_.block_I];
    if (labels_S1D_bsp.empty()) {return;}
    if (blocking_.blockedU_F1D_bsp.size() != labels_S1D_bsp.size()) {return;}
    if (blocking_.blockedV_F1D_bsp.size() != labels_S1D_bsp.size()) {return;}
    ...
    std::cout << " ovlp=" << std::setw(8) << std::fixed << std::setprecision(4) << blocking_.overlap_F;
...
void HFBKramersNucleusCylindrical::print_abstract(int iteration_I, double error_F, double mixing_F) const {
    ...
    observable_.update_observable(*this);
    // Blocking: report the actual chemical potential.
    if (hfb_neutron.blocking_Func) {observable_.lambda_n_F = hfb_neutron.lambda_F;}
    if (hfb_proton.blocking_Func) {observable_.lambda_p_F = hfb_proton.lambda_F;}
    ...
    if (hfbsetting.useLipkinNogami_B) {
    ...
    std::cout << std::setw(12) << std::setprecision(8) << error_F;
