...
int main() {
    ...
    for (int n_I = 0; n_I < Ncheck_I; ++n_I) {
        ...
        errEmax_F = std::max(errEmax_F, err_F);
    }
    ...
    for (int n_I = 0; n_I < Ncheck_I; ++n_I) {
        ...
        errPsimax_F = std::max(errPsimax_F, waveErr_F);
    }
    ...
    for (int n_I = 0; n_I < Ncheck_I; ++n_I) {
        ...
        double Eexact_F = harmonic_oscillator_energy(n_I);
        double err_F = std::abs(E_F - Eexact_F);
        std::cout << std::setw(10) << n_I << std::setw(18) << Eexact_F << std::setw(18) << E_F << std::setw(18) << err_F << "\n";
    }
    ...
    for (int n_I = 0; n_I < Ncheck_I; ++n_I) {
        ...
        double overlapAbs_F = std::abs(overlap_F) / std::sqrt(normNum_F * normExact_F);
        double waveErr_F = 1.0 - overlapAbs_F;
        std::cout << std::setw(10) << n_I << std::setw(18) << 1.0 << std::setw(18) << overlapAbs_F << std::setw(18) << waveErr_F << "\n";
    }
