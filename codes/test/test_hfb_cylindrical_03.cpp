/**
 * @file    test_hfb_cylindrical_03.cpp
 * @author  cailea
 * @date    2026-09-02
 * @brief   Validate weak-pairing Ca49 blocking.
 */
...
/**
 * @brief Validate weak-pairing Ca49 observables against HFBTHO.
 * @math ^{48}Ca → ^{49}Ca(n), C_q^{V0}=-250
 * @output Passed reference assertions.
 */
int main() {
    std::cout << std::scientific << std::setprecision(12) << std::right;

    // (b₀,N_shell,N_r,N_z) ← run_000003.
    ...
    std::cout << "[Input] C_n^{V0}, C_p^{V0} = -250, -250\n";
    ...
    HFBKramersBlocking activeBlocking_ = blockings_S1D_candidate[4];
    hfb_.hfb_neutron.TargetN_I = Nfinal_I;
    hfb_.hfb_proton.TargetN_I = Zfinal_I;
    hfb_.set_blocking([&](std::vector<HFBKramersBlockSolution>& solutions, bool updateTracking_B) {return activeBlocking_.apply_blocking(solutions, updateTracking_B);}, {});
    hfb_.iterate(true);
    HFBCylindricalObservable observable_;
    observable_.update_observable(hfb_, {activeBlocking_}, {});

    // O_ref ← HFBTHO run_000003.
    const HFBCylindricalObservableReference reference_{-425.463058, -10.343656, -15.028469, -0.000000, -0.000000, 0.000000, 0.000000, 3.632518, 3.455211};
