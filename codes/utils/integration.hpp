...
inline double integrate_qag(const RealToRealFunc& f_Func, double xmin_F, double xmax_F) {
    ...
    gsl_Func.function = [](double x_F, void* f_Func_ptr) { return (*static_cast<const RealToRealFunc*>(f_Func_ptr))(x_F); };
...
/**
 * @brief  Integrate a function over the unit sphere.
 * @math   I_Ω = ∫_0^{2π} ∫_0^π f(θ, φ) sin(θ) dθ dφ
 * @output Numerical integral over the unit sphere.
 * @note   Uses Gauss-Legendre quadrature in cos(θ) and trapezoidal quadrature in φ.
 */
