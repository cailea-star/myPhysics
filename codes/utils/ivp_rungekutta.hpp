...
public:
    ...
    : tcurr_F(t0_F), f_Func(f_Func_), ycurr_T1D_i(y0_T1D_i), k1_T1D_i(y0_T1D_i.size()), k2_T1D_i(y0_T1D_i.size()), k3_T1D_i(y0_T1D_i.size()), k4_T1D_i(y0_T1D_i.size()), ytmp_T1D_i(y0_T1D_i.size()) {
        assert(this->f_Func);
...
T ivp_rk4_step(const RealT2TFunc<T>& f_Func, double t_F, T y_T, double dt_F) {
    // (t_n,y_n) → {k_1,k_2,k_3,k_4} → y_{n+1}.
...
/**
 * @brief  Solve scalar grid IVP by RK4.
...
/**
 * @brief  Solve vector grid IVP by RK4.
 * @math   y(t₀)=y₀, y(tᵢ₊₁)=RK4[f,tᵢ,y(tᵢ),tᵢ₊₁-tᵢ]
