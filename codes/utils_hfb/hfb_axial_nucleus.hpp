...
inline void HFBAxialNucleus::iterate(int TargetN_I_, int TargetZ_I_, bool useCurrentFields_B) {
    ...
    const auto calc_Gx_Func = [&](const Eigen::VectorXd& x_F1D_packed_, Eigen::VectorXd& Gx_F1D_packed_) {
        ...
        hfb_axial_neutron.search_lambda(TargetN_I, hfbsetting.temperature_F, EspCut_F, lambdaAccuracy_F);
        hfb_axial_proton.search_lambda(TargetZ_I, hfbsetting.temperature_F, EspCut_F, lambdaAccuracy_F);
        update_Gamma_Delta();
