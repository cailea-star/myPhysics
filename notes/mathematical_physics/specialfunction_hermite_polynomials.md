...
$$
\frac{\partial G}{\partial t}=\sum_{n=1}^{\infty}H_n(x)\frac{t^{n-1}}{(n-1)!}=\sum_{n=0}^{\infty}H_{n+1}(x)\frac{t^n}{n!}=\left(2x-\frac{\partial}{\partial x}\right)G.
$$

Therefore,

$$
\boxed{\frac{\partial G}{\partial t}+\frac{\partial G}{\partial x}=2xG}.
$$
