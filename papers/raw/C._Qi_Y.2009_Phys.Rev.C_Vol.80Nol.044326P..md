---
[source_json]: C._Qi_Y.2009_Phys.Rev.C_Vol.80Nol.044326P..json
...
[tags]: cluster_radioactivity
[source]: section I.
```

> The spontaneous emission of charged fragments heavier than the α particle (cluster decay) was predicted in Ref. [3] and later established experimentally in trans-lead mother nuclei decaying into daughters around the doubly magic nucleus \(^{208}\mathrm{Pb}\) [4, 5, 6, 7].
...
> Besides, one basic assumption behind the relation (12) is that one can define a proper radius \(R'\) that leading to a stable formation amplitude \(F_c(R')\) for all cluster radioactivities.
...
```tags
[claim-type]: definition
[tags]: alpha_formation_amplitude, cluster_formation_amplitude
[source]: section II.
```
...
```tags
[claim-type]: background
[tags]: alpha_decay_half_life
[source]: section I.
```

> The first striking correlation in α decay systematics was noted by Geiger and Nuttall [31]. This relates the decay half-lives \(T_{1/2}\) and decay energies \(Q_\alpha\) as:
...
```tags
[claim-type]: result
[tags]: cluster_formation_amplitude, R_matrix_theory
[source]: section III.A., figure 2.
```
...
```tags
[claim-type]: result
[tags]: alpha_formation_amplitude, cluster_formation_amplitude, UDL
[source]: section III.A., figure 3.
```
...
```tags
[claim-type]: result
[tags]: alpha_decay_half_life, UDL
[source]: section III.B., figure 4.
```
...
```tags
[claim-type]: result
[tags]: alpha_decay_half_life, cluster_decay_half_life, UDL
[source]: section III.B., figures 4–5.
```
...
```tags
[claim-type]: result
[tags]: cluster_decay_half_life, UDL
[source]: section IV., figure 8.
```
...
```tags
[claim-type]: result
[tags]: decay_mode_branching_ratio, UDL
[source]: section IV., table III.
```
...
```tags
[claim-type]: result
[tags]: cluster_decay_half_life, decay_mode_branching_ratio, UDL
[source]: section IV., table V.
```

> With the same selection criteria discussed above, our predictions on probable emissions of \(N_c=Z_c\) clusters are listed in Table V. [...] It is seen from the Table that the mostly likely \(N_c=Z_c\) cluster emitter is the nucleus \(^{114}\mathrm{Ba}\).
...
> The resulting linear expression (Eq. (12)) is found to be a generalization of the Geiger-Nuttall law and we call it the universal decay law (UDL).
...
[source]: section I.
[ref]: [31]
[doi]: 10.1080/14786441008637156; 10.1007/BF01329570
```
...
```tags
[claim-type]: background
[tags]: R_matrix_theory, cluster_radioactivity
[source]: section II.
[ref]: [26, 49]
[doi]: 10.1143/PTP.12.253; 10.1103/RevModPhys.30.257
```
...
```tags
[claim-type]: method
[tags]: UDL, cluster_decay_half_life
[source]: section IV.
[ref]: [52]
...
[source]: section III.
[ref]: [1, 52, 22, 53]
[doi]: 10.1016/j.nuclphysa.2003.11.001; 10.1016/j.nuclphysa.2003.11.003; 10.1103/PhysRevC.77.054318; 10.1103/PhysRevC.76.044326
```
...
```tags
[claim-type]: background
[tags]: decay_mode_branching_ratio
[source]: section IV., table V.
[ref]: [1]
...
[source]: section IV., table II.
[ref]: [1, 55]
[doi]: 10.1016/j.nuclphysa.2003.11.001; unknown
```
...
- `[section]: Results | [item]: coverage | [target]: figure 7 | [reason]: χ′ is tagged only with the UDL method`
- `[section]: Results | [item]: coverage | [target]: figure 11 | [reason]: χ′ is tagged only with the UDL method`
- `[section]: Results | [item]: coverage | [target]: table I | [reason]: outputs are tagged only with the UDL method`
