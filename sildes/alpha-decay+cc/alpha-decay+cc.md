...
![超重核壳修正能分布](.assert/superheavy-shell-correction-map.png)

![已知最重核素及其衰变模式](.assert/heaviest-nuclides.png)

##### 2. α 衰变

> - 在理论上，α 衰变通常描述为：α 团簇首先在母核内部预形成，随后通过量子隧穿穿过 α–子核势垒并发射到核外。
> - 在实验上，通常通过从未知核素逐级衰变到已知核素的级联 α 衰变链鉴别新合成核素；因此，α 衰变链构成探索未知重核区的重要实验观测入口。

![α 衰变势垒与量子隧穿示意图](.assert/alpha-decay-barrier.png)

![超重核半衰期与衰变模式分布](.assert/superheavy-half-life-map.png)

##### 3. 现有研究方法与耦合道动机

> - 目前，α 衰变理论研究大体可分为宏观系统学研究与微观研究两类。
> - 宏观系统学方法，如 VSF 和 UDL，将已有实验规律参数化为经验或半经验公式，通常具有较好的整体预测精度；但核结构信息往往被平均化或吸收进参数，难以揭示内部结构与衰变机制。
> - α 衰变的微观研究主要涉及单粒子高激发态、配对关联和四核子团簇化等物理机制，但目前仍难以精确计算 α 团簇形成振幅。
> - 任中洲、倪冬冬等采用耦合道方法研究 α 衰变精细结构与分支比；我们的工作首先复现该方法，再在此基础上进行改进。

![α 团簇形成振幅与配对能隙](.assert/alpha-formation-amplitude.png)

![α 衰变宏观系统学规律](.assert/alpha-decay-systematics.png)

![耦合道方法计算的 α 衰变分支比](.assert/coupled-channels-branching-ratios.png)
